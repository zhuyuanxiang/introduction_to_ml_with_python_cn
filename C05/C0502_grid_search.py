...
def simple_grid_search():
    ...
    number_title = "简单的网格搜索对 SVC 模型进行评分"
    print('\n', '-' * 5, number_title, '-' * 5)
...
def three_fold_grid_search():
    ...
    number_title = "将数据分割为训练集、验证集、测试集再利用网格搜索对 SVC 模型进行评分"
    print('\n', '-' * 5, number_title, '-' * 5)
    ...
    print("测试集上的最佳得分：{:.5f}".format(test_score))
...
def cross_validation_grid_search():
    ...
    number_title = " 带交叉验证的网格搜索对 SVC 模型进行评分"
    print('\n', '-' * 5, number_title, '-' * 5)
    ...
    for C in [0.001, 0.01, 0.1, 1, 10, 100]:
        for gamma in [0.001, 0.01, 0.1, 1, 10, 100]:
            ...
            # 计算交叉验证的平均精度
            # score = scores.mean 不能使用这个方法，因为返回的是np.array
...
def grid_search_cv():
    ...
    number_title = "使用 Scikit-Learn 中 GridSearchCV() 对 SVC 模型进行评分"
    print('\n', '-' * 5, number_title, '-' * 5)
...
def non_grid_search_cv():
    ...
    number_title = "使用 GridSearchCV() 的参数列表对 SVC 模型进行评分"
    print('\n', '-' * 5, number_title, '-' * 5)
    ...
    print("网格列表：{}".format(param_grid_list))
...
def grid_search_with_different_strategies():
    ...
    number_title = "使用 GridSearchCV() 的参数列表+交叉验证对 SVC 模型进行评分"
    print('\n', '-' * 5, number_title, '-' * 5)
    ...
    # 重复分层K折交叉验证分离器（RepeatedStratifiedKFold），进一步增加数据分布的随机性
    cv = RepeatedStratifiedKFold(5)

    grid_search = GridSearchCV(SVC(), param_grid_list, iid = True, cv = cv)

    from sklearn.model_selection import cross_val_score
    # scores = cross_val_score(grid_search, iris.data, iris.target, cv = 5)
    scores = cross_val_score(grid_search, iris.data, iris.target, cv = cv)
