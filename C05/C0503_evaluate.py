...
def compare_confusion_matrix():
    ...
    number_title = "使用混淆矩阵分析不平衡的数据对模型的影响"
    print('\n', '-' * 5, number_title, '-' * 5)
    print("数据中90%都是False，只有10%的数据是True，因此只要预测False，90%都是正确的")
...
def compare_f1_score():
    ...
    number_title = "使用F1分数分析不平衡的数据对模型的影响"
    print('\n', '-' * 5, number_title, '-' * 5)
    print("数据中90%都是False，只有10%的数据是True，因此只要预测False，90%都是正确的")
...
def compare_classification_report():
    ...
    number_title = "使用分类报告分析不平衡的数据对模型的影响"
    print('\n', '-' * 5, number_title, '-' * 5)
    ...
    predict_logistic_regression = logistic_regression.predict(X_test)
    print('-' * 20)
    print("Logistic Regression 模型的分类报告:")
...
def imbalanced_two_classes():
    ...
    X, y = make_blobs(n_samples = (350, 50), centers = 2, cluster_std = [7.0, 2], random_state = 22)

    number_title = "不平衡数据的二分类问题"
    print('\n', '-' * 5, number_title, '-' * 5)
...
def compare_precision_recall_curve():
    ...
    X, y = make_blobs(n_samples = (3500, 500), n_features = 2, cluster_std = [7.0, 2], random_state = 22)

    number_title = "使用准确率——召回率曲线分析不平衡的数据对模型的影响"
    print('\n', '-' * 5, number_title, '-' * 5)
    print("数据中87.5%是一类，12.5%的数据是另一类")
    ...
    plt.ylabel('召回率')
    plt.legend()
    plt.suptitle("图5-14：比较 SVM 与 随机森林 的 准确率--召回率曲线\n"
                 "SVM在中间位置的表现更好，随机森林在极值处表现更好（高准确率或高召回率）")

    print('-' * 20)
    print("f1_score表示了准确率——召回率曲线上默认阈值对应的点")
    from sklearn.metrics import f1_score
    predict_svc = svc.predict(X_test)
    ...
    print('=' * 20)
    print("平均准确率（Average Precision）表示曲线下的积分（即面积）")
...
def compare_roc_curve():
    ...
    number_title = "使用 ROC 曲线分析不平衡的数据对模型的影响"
    print('\n', '-' * 5, number_title, '-' * 5)
    print("数据中87.5%是一类，12.5%的数据是另一类")
    ...
    plt.legend(loc = 4)
    plt.suptitle("图5-15：SVC（gamma=0.05）的ROC曲线\n"
                 "曲线越靠近左上角，则分类器越好")
    ...
    plt.ylabel('TPR（真真类率）')
    plt.legend(loc = 4)
    plt.suptitle("图5-16：比较 SVM 和 随机森林 的 ROC曲线\n"
                 "曲线越靠近左上角，则分类器越好\n"
    ...
    print('=' * 20)
    print("AUC 表示曲线下的积分（即面积），解释为评估正例样本的排名")
...
def compare_svc_gamma_roc():
    ...
    number_title = "对比 SVC 模型中不同 gamma 值对 ROC 曲线的影响"
    print('\n', '-' * 5, number_title, '-' * 5)
    print("数据中90%都是False，只有10%的数据是True，因此只要预测False，90%都是正确的")
    ...
    for gamma in [1, 0.5, 0.1, 0.05, 0.01]:
        ...
        print('-' * 20)
        print("gamma = {:.2f}\t 精确度 = {:.2f}\t AUC = {:.2f}".format(gamma, accuracy, auc))
    ...
    plt.ylim(0, 1.02)
    plt.legend(loc = "best")
    plt.suptitle("图5-17：对比不同gamma值的SVM的ROC曲线")
    pass
...
def multi_classes_score():
    ...
    plt.suptitle("图5-18：10个数字分类任务的混淆矩阵")

    print('-' * 20)
    from sklearn.metrics import f1_score
    print("10个数字分类任务的微平均：{:.3f}".format(f1_score(y_test, predict_logistic_regression, average = 'micro')))
    print("10个数字分类任务的宏平均：{:.3f}".format(f1_score(y_test, predict_logistic_regression, average = 'macro')))
    print("10个数字分类任务的加权平均：{:.3f}".format(f1_score(y_test, predict_logistic_regression, average = 'weighted')))
    from sklearn.metrics import classification_report
    print("10个数字分类任务的分类报告")
...
def model_selection_with_score():
    ...
    from sklearn.svm import SVC
    print("交叉验证的精确度评估指标：")
    print("交叉验证的默认评估指标是“scoring='accuracy'”")
    print("Accuracy scoring:", cross_val_score(SVC(gamma = 'auto'), digits.data, y, scoring = 'accuracy', cv = 5))
    print('-' * 20)
    print("交叉验证的指定评估指标是“scoring = 'roc_auc'”")
    print("AUC scoring: ", cross_val_score(SVC(gamma = 'auto'), digits.data, y, scoring = 'roc_auc', cv = 5))
    ...
    print('=' * 20)
    print("网格搜索的评估指标：", grid_search.scoring)
    print('-' * 20)
